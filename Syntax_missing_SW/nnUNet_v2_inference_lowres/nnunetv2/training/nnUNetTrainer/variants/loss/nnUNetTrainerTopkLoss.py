...
class nnUNetTrainerTopk10Loss(nnUNetTrainer):
    print("nnunetv2.training.nnUNetTrainer.variants.loss.nnUNetTrainerTopkLoss.nnUNetTrainerTopk10Loss")
...
class nnUNetTrainerTopk10LossLS01(nnUNetTrainer):
    print("nnunetv2.training.nnUNetTrainer.variants.loss.nnUNetTrainerTopkLoss.nnUNetTrainerTopk10LossLS01")
    def _build_loss(self):
        assert not self.label_manager.has_regions, 'regions not supported by this trainer'
        loss = TopKLoss(ignore_index=self.label_manager.ignore_label if self.label_manager.has_ignore_label else -100,
...
class nnUNetTrainerDiceTopK10Loss(nnUNetTrainer):
    print("nnunetv2.training.nnUNetTrainer.variants.loss.nnUNetTrainerTopkLoss.nnUNetTrainerDiceTopK10Loss")
    def _build_loss(self):
        assert not self.label_manager.has_regions, 'regions not supported by this trainer'
